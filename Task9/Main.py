...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.Cutting = Polygon()
        self.CuttingDone = False
    ...
    def connectButtons(self):
        ...
        self.CloseCuttingButton.clicked.connect(self.CloseCuttingSlot)
    ...
    def keyPressEvent(self, e):
        ...
        elif e.key() == QtCore.Qt.Key.Key_Alt:
    ...
    def mouseReleaseEvent(self, a0: QtGui.QMouseEvent | None) -> None:
        if self.DrawLabel.IsPosInside(a0.x(), a0.y()) and a0.button() == QtCore.Qt.MouseButton.LeftButton:
            if self.AltPressed and len(self.Cutting) > 0 and not self.CuttingDone and len(self.Polygon) > 0:
                min_p = min([p for p in self.Polygon.points], key=lambda p: Point(a0.x() - p[0], a0.y() - p[1]).vectorLength())
                self.addLinePoint(min_p)
            elif self.CtrlPressed and len(self.Cutting) > 0 and not self.CuttingDone:
                vec = Point(a0.x() - self.Cutting.points[-1].x, a0.y() - self.Cutting.points[-1].y)
                if abs(vec.x) >= abs(vec.y):
                    self.addLinePoint(Point(a0.x(), self.Cutting.points[-1].y))
                else:
                    self.addLinePoint(Point(self.Cutting.points[-1].x, a0.y()))
            elif self.ShiftPressed and len(self.Polygon) > 2 and len(self.Cutting) > 0 and not self.CuttingDone:
                vec = Point(a0.x() - self.Cutting.points[-1].x, a0.y() - self.Cutting.points[-1].y)
                ...
                if closestVec[0] == 0:
                    ...
                    if (abs(vec[0]) > abs(vec[1])):
                        self.addLinePoint(Point(a0.x(), mathRound(self.Cutting.points[-1].y + closestVec[1] / closestVec[0] * (a0.x() - self.Cutting.points[-1].x))))
                    else:
                        self.addLinePoint(Point(mathRound(self.Cutting.points[-1].x + closestVec[0] / closestVec[1] * (a0.y() - self.Cutting.points[-1].y)), a0.y()))
            else:
    ...
    def CloseCutting(self):
        if (len(self.Cutting) < 3):
            msg = QMessageBox()
            msg.setWindowTitle("Ошибка")
            msg.setText("У многоулольника не может быть меньше 3х точек")
            msg.exec_()
            return
        self.CuttingDone = True
        self.redrawScene()
    
    def CloseCuttingSlot(self):
        self.CloseCutting()
    ...
    def Cut(self):
        if not self.PolygonDone:
            ...
            msg.exec_()
            return
        if not self.CuttingDone:
            msg = QMessageBox()
            msg.setWindowTitle("Ошибка")
            msg.setText("Отсекаемый многоугольник должен быть замкнут")
            msg.exec_()
            return
        self.DrawLabel.setPen(self.CuttedPen)
        cutted = self.Polygon.SutherlandHojmann(self.Cutting)
        # print(*cutted.points, sep="\n")
        for i in range(-1, len(cutted) - 1):
            self.drawLine((cutted[i], cutted[i + 1]), width=2)
    ...
    def drawCutting(self):
        for i in range(len(self.Cutting) - 1):
            self.DrawLabel.DrawLine(self.Cutting[i], self.Cutting[i + 1])
        if (self.CuttingDone):
            self.DrawLabel.DrawLine(self.Cutting[-1], self.Cutting[0])
